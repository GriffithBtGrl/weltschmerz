...
const getPosts = async (req, res, next) => {
  try {
    ...
    console.log('user en getPosts:', req.user);

    const { board, sort = "new", page = 1, limit = 20 } = req.query;
    const offset = (page - 1) * limit;
    const orderBy =
      sort === "popular" ? "p.vote_score DESC" : "p.created_at DESC";
    ...
    if (board) {
      queryText = `
        ...
        LIMIT $1 OFFSET $2`;
      params = [limit, offset, board, user_id];
    } else {
      queryText = `
        SELECT 
          p.id, p.title, p.content, p.image_url, p.vote_score, p.upvotes, p.downvotes,
        ...
        JOIN boards b ON p.board_id = b.id
        LEFT JOIN users u ON p.user_id = u.id
        LEFT JOIN votes v ON v.post_id = p.id AND v.user_id = $3
        ORDER BY p.is_pinned DESC, ${orderBy}
        LIMIT $1 OFFSET $2`;
...
const createPost = async (req, res, next) => {
  try {
    ...
    await query("UPDATE boards SET post_count = post_count + 1 WHERE id = $1", [
      board_id,
    ]);
