...
const NotificationBell = () => {
  ...
  const panel = open ? createPortal(
    <div
      ...
      className="fixed inset-x-0 top-14 bottom-0 md:fixed md:inset-auto md:right-4 md:top-16 md:w-80 md:max-h-96 md:rounded-lg border-t md:border border-dark-600 shadow-lg z-[9999] flex flex-col overflow-hidden"
    >
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => (
            <Link
              ...
              className={`block px-4 py-3 border-b border-dark-700 hover:bg-dark-700/50 transition-colors ${!n.read ? 'bg-dark-700/30' : ''}`}
            >
